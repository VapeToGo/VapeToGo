...
const MASTER_PASSWORD = '123456'; // كلمة السر الرئيسية المعتمدة في Apps Script
...
async function loadMerchantsList() {
  ...
  try {
    const response = await fetch(`${SCRIPT_URL}?action=getMerchants&masterPassword=${MASTER_PASSWORD}`);
    const data = await response.json();
...
async function addNewMerchant(event) {
  ...
  try {
    const url = `${SCRIPT_URL}?action=addMerchant&masterPassword=${MASTER_PASSWORD}&username=${encodeURIComponent(username)}&password=${encodeURIComponent(password)}&merchantName=${encodeURIComponent(merchantName)}`;
...
async function toggleMerchantHold(username) {
  ...
  try {
    const response = await fetch(`${SCRIPT_URL}?action=toggleMerchantStatus&masterPassword=${MASTER_PASSWORD}&username=${encodeURIComponent(username)}`);
    const result = await response.json();
    if (result.status === 'success') {
...
async function deleteMerchantAdmin(username) {
  ...
  try {
    const response = await fetch(`${SCRIPT_URL}?action=deleteMerchant&masterPassword=${MASTER_PASSWORD}&username=${encodeURIComponent(username)}`);
    const result = await response.json();
...
async function deleteProductAsAdmin(id) {
  ...
  try {
    const response = await fetch(`${SCRIPT_URL}?action=deleteProduct&masterPassword=${MASTER_PASSWORD}&id=${encodeURIComponent(id)}`);
    const result = await response.json();
    if (result.status === 'deleted') {
